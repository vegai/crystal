...
- Symbol-table versioning emits a fresh `:symbol_table:vN` LLVM array global per dirty submission (`codegen.cr#define_symbol_table` + `symbol_table_name`), so older arrays linger in the LLJIT dylib until session disposal. Memory grows monotonically across submissions in proportion to `(symbols × dirty-submission count)`. Interactive use stays bounded in practice; a real fix would route old arrays through ORC's unlink path before adding the new one.
- `1.class` returns `Int32.metaclass` under JIT (matching compiled Crystal: `1.class.to_s` is `"Int32"`) but the bytecode interpreter returns the instance type directly. `spec/compiler/interpreter/types_spec.cr#interprets class for non-union type` papers over the divergence by normalising metaclass→instance before the assertion; a real fix would align the bytecode `.class` semantics with JIT/compiled.
